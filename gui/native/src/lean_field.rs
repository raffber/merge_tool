...
pub struct LeanField<T: 'static + Clone + Default> {
    value: T,
    version: Id,
    valid: bool,
...
impl<T: 'static + Clone + Default> LeanField<T> {
    ...
    pub fn update(&mut self, value: &mut T, msg: LeanMsg) -> (bool, Updated) {
        match msg {
            LeanMsg::KeyUp(evt) => {
    ...
    pub fn render(&self) -> ElementBuilder<LeanMsg> {
        let render_fun = "{
