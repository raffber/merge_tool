...
fn main() {
    ...
    let output_dir = match canonicalize(output_dir) {
        Ok(x) => x,
        Err(err) => {
            println!("Invalid output directory: {}", err);
            return;
        },
    };
